...
$$ 
\text{where} \quad \text{Cov}(\mcol{g_c^{m}}, \pcol{g_c^{p}}) = \sum_{j=1}^{M}\sum_{k=1}^{M} \beta_j\beta_k\,\text{Cov}(\mcol{x_j^{m}}, \pcol{x_k^{p}}) 
$$
:::
:::
...
Suppose that a relative $\ell$ (e.g., a parent) environmentally influence the focal person's phenotype, and that this influence was heritable on behalf of the relative:

$$
\underbrace{
y_i
=
\sum_{j=1}^{M} \beta_j x_{ij}
+
e_i
}_{\text{additive model}}
\quad\text{where}\quad
\underbrace{
e_i = \sum_\ell e_{i \ell}^{R} + e_i^\prime}_{\text{environmental effects}}
\quad\text{and}\quad
\underbrace{
e_{i \ell}^{R} = \sum_{k=1}^{M}\gamma_{k \ell} x_{i\ell k} + e_{i\ell}}_{\text{relative's effect}}
$$
...
## Key idea:
By conditioning on parents' or siblings' genotype, you remove the variance that is predictable from the parents, and are left with only the random segregeation variance. <br>This variance is independent from long-range LD and environmental effects.
:::
...
$$
\mathbf{y} = \mathbf{g}_c + \mathbf{g}_m + \mathbf{e},
\qquad
\begin{pmatrix}
\mathbf{g}_c \\
\mathbf{g}_m
...
\end{pmatrix},
\begin{pmatrix}
\sigma_d^2 \mathbf{G}_{oo} & \sigma_{di} \mathbf{G}_{om} \\
\sigma_{di} \mathbf{G}_{mo} & \sigma_i^2 \mathbf{G}_{mm}
\end{pmatrix}
\right)
$$
...
- a direct component based on $\mathbf{G}_{oo} =$ `Goo`
- a maternal indirect component based on $\mathbf{G}_{mm} =$ `Gmm`
- a covariance component for overlap between direct and maternal effects ($\textbf{G_om + Gmo}$): `Dom`
